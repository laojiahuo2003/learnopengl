...
int main()
{
    ...
    // ------------------------------------
    Shader lightingShader("shaders/大作业.vs", "shaders/大作业.fs");
    Shader lightCubeShader("shaders/多光源.vs", "shaders/多光源.fs");

    // set up vertex data (and buffer(s)) and configure vertex attributes
    // ------------------------------------------------------------------
    float vertices[] = {
        // 三个位置坐标           // 三个法向量normals           // 两个纹理坐标        //三个控制点
...
// 在渲染循环之前定义
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 计算变化的时间增量
        totalTime += deltaTime;
        float change = sin(totalTime) * 0.5f + 0.5f; // 让 change 在 0 和 1 之间变化
        // input
        // -----
        processInput(window);
        ...
        lightingShader.setFloat("change", change);
